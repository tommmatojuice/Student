...
//        users_table.setItems(usersManager.getAll());
    ...
    private void addUser() throws SQLException {
        if(master_enter.getValue() != null){
            Optional<ButtonType> option = dialogUtil.showConfirmMessage("Добавить запись", "Вы действительно хотите добавить запись?", "Пользователь").showAndWait();
            if (option.get() == ButtonType.OK) {
                try {
    ...
    private void copyToClipboard(Users user){
        Optional<ButtonType> option = dialogUtil.showMyMessage("Добавлен пользователь", null, user.toString()).showAndWait();
        if (option.get() != null) {
