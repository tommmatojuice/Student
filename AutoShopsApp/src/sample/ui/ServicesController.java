...
import data.manager.ServicesManager;
...
//                consum_button, work_button, cintract_button, service_button, math_button, users_button);
    ...
    private void setTable() throws SQLException {
        ...
        try{
            ...
            dialogUtil.showErrorMessage("Ошибка", "Неверно введена стоимость!");
        }
    }
    ...
    private void addService(){
        if(!type_enter.getText().isEmpty() && !price_enter.getText().isEmpty()){
            if (checkUtil.priceCheck(price_enter.getText())){
                Optional<ButtonType> option = dialogUtil.showConfirmMessage("Добавить запись", "Вы действительно хотите добавить запись?", "Услуга").showAndWait();
                if (option.get() == ButtonType.OK) {
                    try {
