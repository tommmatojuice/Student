...
public class CustomerController {
    ...
    private void setTable() throws SQLException {
        ...
        nameColumn.setOnEditCommit((TableColumn.CellEditEvent<Customer, String> event) -> {
            ...
            if(checkUtil.fullNameCheck(event.getNewValue())){
                ...
                dialogUtil.showErrorMessage("Ошибка", "ФИО введены неверно!");
                try {
            ...
            if (checkUtil.phoneCheck(event.getNewValue())){
                try {
                    if(!customerManager.getByPhone(event.getNewValue())){
                        ...
                        changeCheck(customer, event.getNewValue());
                    } else dialogUtil.showErrorMessage("Ошибка", "Клиент с таким номером телефона уже существует!");
                } catch (SQLException throwables) {
                    throwables.printStackTrace();
                }
            } else {
        ...
        passportColumn.setOnEditCommit((TableColumn.CellEditEvent<Customer, String> event) ->{
            TablePosition<Customer, String> pos = event.getTablePosition();
            Customer customer = event.getTableView().getItems().get(pos.getRow());
            if (checkUtil.passportCheck(event.getNewValue())){
                customer.setPassport(event.getNewValue());
                changeCheck(customer, event.getNewValue());
            } else {
                dialogUtil.showErrorMessage("Ошибка", "Неверные серия и номер паспорта!");
                try {
                    customers_table.setItems(customerManager.getAll());
                } catch (SQLException throwables) {
                    throwables.printStackTrace();
    ...
    private void addCustomer() throws SQLException {
        if(!name_enter.getText().isEmpty() && !address_enter.getText().isEmpty() && !phone_enter.getText().isEmpty() && !passport_enter.getText().isEmpty()){
            if (checkUtil.phoneCheck(phone_enter.getText())){
                if (checkUtil.passportCheck(passport_enter.getText())){
                    if(checkUtil.fullNameCheck(name_enter.getText())){
                        if(!customerManager.getByPhone(phone_enter.getText())){
                            Optional<ButtonType> option = dialogUtil.showConfirmMessage("Добавить запись", "Вы действительно хотите добавить запись?", "Клиент").showAndWait();
                            if (option.get() == ButtonType.OK) {
                                try {
                    ...
                    } else dialogUtil.showErrorMessage("Ошибка", "ФИО введены неверно!");
                } else dialogUtil.showErrorMessage("Ошибка", "Серия и номер паспарта должно состоять из 10 символов!");
    ...
    public void goToCars(Customer customer) throws IOException, SQLException {
        Optional<ButtonType> option = dialogUtil.showConfirmMessage("Добавить автомобиль", "Добавить автомобить клиента?", customer.toString()).showAndWait();
        if (option.get() == ButtonType.OK) {
            try {
